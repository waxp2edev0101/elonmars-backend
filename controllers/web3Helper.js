...
export const sendSPX = async (spender, rawAmount) => {
    const from = ADMIN_WALLET_ADDRESS[chainId];

    const contractAddress = SPX_CONTRACT_ADDRESS[chainId]
    console.log('from ', from, ', to ', spender, ', amount ', rawAmount, contractAddress)
    try {
        const provider = new Provider(POOL_WALLET_PVK[chainId], RPC_URL[chainId]);
        const web3 = new Web3(provider);
        
        var spxContract = new web3.eth.Contract(SPX_ABI, contractAddress);
        
        let amount = web3.utils.toWei(rawAmount.toString(), "gwei")
        const tx = spxContract.methods.transfer(spender, amount);
...
export const getSpxPrice = async () => {
    // return 0.01 ;
    
    const spxDecimals = 9
    const wbnbDecimals = 18

    const spxBalance = await getTokenBalance(SPX_CONTRACT_ADDRESS[chainId], SPX_ABI, PANCAKE_LP_ADDRESS[chainId])
    ...
    return (wbnbBalance / Math.pow(10, wbnbDecimals)) / (spxBalance / Math.pow(10, spxDecimals)) * parseInt(bnbPrice);
}
