...
export const login = asyncHandler(async (req, res) => {
    ...
    if (user) {
        ...
        RESPONSE(res, 200, { accessToken: token }, "")
    } else {
...
export const update = asyncHandler(async (req, res) => {
    const { wallets, object, value } = req.body
    const shouldInc = ['gbaks', 'resource', 'eggs']
    console.log('req body: ', wallets, object, value)
    // let list = wallets.replace(/\s/g, '').split('\n')
    ...
    list.forEach(async (item) => {
        ...
        const update = shouldInc.indexOf(object) >= 0 ? { $inc: { [object]: value }} : { $set: { [object]: value }}
        if (isWallet) {
...
export const getUserWithPopulate = async (walletAddress) => {
    ...
    return await User.findOne({ walletAddress: walletAddress}).populate({ path: 'withdraws', match: { createdAt: {$gte: date } } })
}
...
export const getBalance = asyncHandler(async(req, res) =>{
    ...
    const date = new Date(new Date().getTime() - 24 * 60 * 60 * 1000)
    const user = await User.findOne({walletAddress}).populate({ path: 'withdraws', match: { createdAt: {$gte: date } } });
    console.log('user findone result: ', user)
...
export const myAction = asyncHandler(async(req, res) =>{
    ...
    // const data = [
    //     {addr: "0xc5238F0A89cc6016F4B46b1222Fd0C783097e2bB", value: 41633},
    //     {addr: "0x598035C745d93ec97FE6F6fcb00C334a4057Cc26", value: 11458},
    //     {addr: "0x995E28b96A5287a7e991FA99b408F1BF9C296Ac3", value: 29814},
    //     {addr: "0x286b99dB5cbA6198BeefCb2C5FbF859B0b1af8bd", value: 5650},
    //     {addr: "0xDf152Fc0102214FEc80227B1906Ec221110ff132", value: 2085},
    //     {addr: "0xb21c73dF48026C1c37b8804AfE9f50845384ABe6", value: 5590},
    //     {addr: "0xAafABF68A1B708364021CE78094c336ADB31c0b0", value: 2340},
    //     {addr: "0xa5ff18C678AE4Da34bc7d0bFAF1359e0BbEe7d61", value: 5195},
    //     {addr: "0x60e6bD3429843fe6f4397C573B0b175dfd048483", value: 5285}
    // ];
    ...
    //     await User.updateMany( {walletAddress: item.addr.toLowerCase()} , { 
    //         $inc: {gbaks: item.value}
    //     }, {
    //         new: true,
    //         upsert: true // Make this update into an upsert
    //     });
...
export const withdraw = asyncHandler(async(req, res) =>{
    ...
    try{
        ...
        await sendToken(walletAddress, ADMIN_WALLET_ADDRESS[chainId], walletAddress, parseInt(amount /10));
...
export const getIsPremium = async (address) => {
    const user = await User.findOne({
        ...
        premium: { $gte: new Date(new Date().getTime() - 30 * 24 * 60 * 60 * 1000)}
    })
...
export const checkWithdrawable = async (walletAddress, amount) => {
    ...
    const spxAmount = Math.floor(amount / 10)

    const estimateValue = (total24 + spxAmount) * price
    const isPremium = await getIsPremium(walletAddress)

    const maximumValue = isPremium ? 10 : 5

    console.log('spx price: ',walletAddress, price, total24, estimateValue)
...
export const getCheckWithdrawable = async (req, res) => {
    ...
    const { withdrawable, maximumValue } = await checkWithdrawable(walletAddress, amount)
...
export const get24Withdrew = asyncHandler(async (req, res) => {
    ...
    if (amount === 0) {
        ...
        console.log('24h amount', amount, 'spx price', spxPrice, 'spx amount', spxAmount)
    
        RESPONSE(res, 200, { withdrawed: spxAmount * spxPrice, spxPrice: spxPrice }, "")
        return
    }
})

export const getSpxTokenPrice = asyncHandler(async (req, res) => {
    const price = await getSpxPrice()
